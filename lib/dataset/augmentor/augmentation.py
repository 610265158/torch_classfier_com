...
def Img_dropout(src,max_pattern_ratio=0.05,with_cover=1.):
    ...
    if random.uniform(0,1)>with_cover:

        random_pitch=np.random.rand((height_end-height_start), (width_end-width_start),3)*255
        src[height_start:height_end, width_start:width_end, :] = np.array(random_pitch,dtype=src.dtype)
    else:
...
def Random_crop(src,shrink):
    ...
    start_h=random.randint(0,int(h_shrink))
    start_w=random.randint(0,int(w_shrink))

    end_h = -random.randint(1,int(h_shrink))
    end_w = -random.randint(1,int(w_shrink))
