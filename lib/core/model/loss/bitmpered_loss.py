...
def bi_tempered_logistic_loss(activations, labels, t1, t2, label_smoothing=0.0, num_iters=5):
    ...
    C = activations.size(1)
    class_mask = activations.data.new(N, C).fill_(0)
    ...
    return torch.sum(loss_values)/N

import torch.nn as nn

class biloss(nn.Module):

    def __init__(self, t1=0.8, t2=1.4,smooth=0.2):
    ...
    def forward(self, y_pred, y):
        ...
        xx=bi_tempered_logistic_loss(y_pred,y,label_smoothing=self.smooth,t1=self.t1,t2=self.t2)
