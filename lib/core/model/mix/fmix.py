...
class FMix(FMixBase):
        ...
        .. code-block:: python
            class FMixExp(pl.LightningModule):
                ...
                def training_step(self, batch, batch_idx):
                    ...
                    return loss
    """
    def __init__(self, decay_power=3, alpha=1, size=(512, 512), max_soft=0.0, reformulate=False,loss_function=None):
    ...
    def fmix_loss(self,input, y1, index, lam, train=True, reformulate=False):
        ...
        if train and not reformulate:
            y2 = y1[index]
            return self.loss_func(input, y1) * lam + self.loss_func(input, y2) * (1 - lam)
        else:
            return self.loss_func(input, y1)
